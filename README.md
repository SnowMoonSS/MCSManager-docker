...
> [!NOTE]
> 将主机的文件夹映射到容器中前，如果文件夹不存在，请先手动创建，否则 Docker 会自动创建，而 Docker 自动创建的文件夹权限是归属于 root 账户的。
...
本项目提供 6 种变体（含无 Java 精简版），可灵活适配不同 Minecraft 版本需求。官方镜像同样支持通过不同镜像名选择 Java 版本。
...
4. **多 JDK 变体选择**：6 种 JRE 选项覆盖所有 Minecraft 版本需求，且提供无 JRE 的精简版本用于纯 Steam 游戏或只使用 Docker 运行实例的管理场景。
...
services:
  mcsm-daemon:
    image: ghcr.io/snowmoonss/mcsm-daemon:latest          # 不内置 JRE
    # image: ghcr.io/snowmoonss/mcsm-daemon:latest-jre17  # 内置 JRE 17
    # image: ghcr.io/snowmoonss/mcsm-daemon:latest-jre8   # 内置 JRE 8
...
| `latest-jdk17` | JRE 17 | Minecraft 1.17 - 1.20 |
| `latest-jdk11` | JRE 11 | Minecraft 1.8 - 1.17 |
| `latest-jdk8` | JRE 8 | Minecraft 1.16 及更早、Forge 旧版本 |
